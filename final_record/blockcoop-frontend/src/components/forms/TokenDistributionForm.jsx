...
export default function TokenDistributionForm() {
  ...
  const loadBalance = async (tokenAddress) => {
    try {
      ...
      const balance = await contractService.getTokenBalance(tokenAddress, ownerAddress);
      setBalances(prev => ({
        ...prev,
  ...
  return (
    <div className="p-4 border rounded-lg mb-8 shadow-md bg-gray-800">
      <h3 className="text-xl font-semibold mb-4 text-white">Distribute Tokens</h3>
      ...
      <div className="mb-4 p-4 bg-gray-700 rounded-lg">
        <h4 className="text-lg font-medium text-white mb-2">Available Balances</h4>
        <div className="space-y-2">
          {tokens.map(token => (
            <div key={token.address} className="flex justify-between items-center text-gray-300">
              <span>{token.symbol}:</span>
              <span>
                {balances[token.address] 
                  ? `${balances[token.address].formatted} ${balances[token.address].symbol}`
                  : 'Loading...'}
              </span>
            </div>
          ))}
